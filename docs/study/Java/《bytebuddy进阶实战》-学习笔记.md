...
1. 为什么要求传递拦截器类名，而不是拦截器类`Class`对象？
   ...
   + 字节码文件的**字节数组**被**类加载器**加载到JVM中才会产生对应的**`Class<?>`对象**
   + 在Agent扫描插件目录并加载插件jar之前，我们并不知道插件jar使用的类加载器是啥

   + 运行时Agent后续才临时使用**自定义的类加载器**加载插件jar中的插件类和拦截器类

   + 插件jar无法在自己的jar包内感知运行时自己被什么类加载器加载，所以无法预先得到插件类和拦截器类的`Class<?>`对象

   **综上，由于插件jar的拦截器类需要等到运行时才能被Agent的自定义类加载器加载，无法预先加载并获得`Class<?>`，所以`apm-agent-core`中拦截点定义要求插件传递拦截器全限制类名而不是`Class<?>`**
...
实现一个简易版本的SkyWalking的Java Agent可插拔插件加载，可以通过以下方面进行思考和设计：

1. 怎么规范每个插件的实现？

   + 可以定义顶级抽象类/接口，并且要求每个插件实现抽象类/接口的抽象方法
...
2. 每个插件应该提供哪些信息？
   ...
   + 当前拦截的类范围下，需要拦截的方法范围
   + 当前类范围、方法范围下，使用的拦截器
   + 对Agent暴露插件类信息（可以通过约定的配置文件等形式）

3. 如何简化开发者提供拦截类范围的方式？

   + 提供类匹配器抽象类/接口，且预留一些常见实现，比如全限制类名匹配，注解匹配等

4. 如何简化开发者提供拦截器类的方式？

   + 不让开发者提供Byte Buddy原生的拦截器类，而是提供拦截器模版类实现，并且只要求按照AOP风格实现拦截器抽象类/接口。此时开发者只需要聚焦关注具体的增强逻辑实现

5. AOP风格的拦截器增强逻辑填充，怎么传递中间过程值？

   + 可以在实例方法/构造方法增强时，给类第一次增强时添加成员变量字段，用来给拦截器传递中间过程值。比如`beforeMethod`，`afterMethod`分别在被拦截方法前后执行，可通过该成员变量传递中间值。

6. 插件和拦截器的类加载的注意点？

   + 由于插件jar统一放在自定义的类加载路径中(这里视频教程演示中，放在`/dist/plugins`），而JVM自带的类加载器并不知道该类加载路径，所以需要使用自定义的类加载器加载插件类
   + 拦截器类需要使用和被拦截的类相同的类加载器或者使用被拦截的类的类加载器的子类，这样拦截器类才能正常访问被拦截的类

7. 自定义的类加载器需要注意重写哪些方法？

   + `protected Class<?> findClass(String className)`

     通常至少重写该方法。内部从特定的位置获取类字节码对应的字节数据，然后通过`java.lang.ClassLoader#defineClass(java.lang.String, byte[], int, int)`方法将字节数组转成`Class<?>`返回。

   + `public URL getResource(String name)`

     若类加载的路径非JVM自带类加载器识别的路径，那么需要重写该方法，将指定的全限制类名解析成对应的类文件路径URL返回

   + `public Enumeration<URL> getResources(String name)`

     和`public URL getResource(String name)`重写的原因相同，只不过返回的是URL集合
