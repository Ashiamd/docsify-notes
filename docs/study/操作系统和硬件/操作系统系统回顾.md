...
​	其实和创建子进程没什么太大区别，不过是需要自己刻意地"取消COW"，让子进程资源独立于原父进程，且需要让原父进程提前终止，好让子进程交由INIT进程管理。

​	**常见手段，`fork`两次，仅保留父进程和第一次`fork`得到的子进程进一步`fork`的孙子进程，即守护进程**，中间的子进程产物直接`exit()`。<small>=>(fork两次是为了让父进程继续执行自己的逻辑且不需要再和守护进程有任何联系。要是父进程创建守护进程后就没有其它事了，那完全可以只fork一次，然后原进程就直接exit，让守护进程被INIT管理）</small>

​	<small>之所以`fork`两次，是因为任何进程要被`init`接管的前提是原进程消亡。而我们不希望远进程`fork`出守护进程后就直接消亡，而是希望其创建守护进程后，能继续往下执行其它逻辑，所以干脆`fork`两次，让孙子进程（其父进程是原进程的子进程）当守护进程。这样子进程`fork`一执行完就可以直接`exit`使得孙子进程失去父进程而被`init`接管，然后我们原进程也只需要子进程一结束，就能继续往下执行，无需操作守护进程的`exit()`没人收尸=>已经交给INIT管理了。</small>
...
1. 什么是DMA
   ...
   ​	DMA(Direct Memory Access，直接存储器访问) 是所有现代电脑的重要特色，它允许不同速度的硬件装置来沟通，而不需要依赖于 CPU 的大量中断负载。否则，CPU 需要从来源把每一片段的资料复制到[暂存器](https://baike.baidu.com/item/暂存器/4308343)（寄存器），然后把它们再次写回到新的地方。在这个时间中，CPU 对于其他的工作来说就无法使用。
...
3. **若进程睡眠在可中断的优先级上**，则当它收到一个**要忽略的信号**时，该进程被唤醒，但不做`longjmp`，一般是**继续睡眠**。但<u>用户感觉不到进程曾经被唤醒，而是象没有发生过该信号一样</u>。
...
4. 内核对**子进程终止**（**SIGCLD**）信号的处理方法与其他信号有所区别。
   ...
   + **如果父进程执行了系统调用wait**，进程将从系统调用wait中醒来并返回wait调用，执行一系列wait调用的后续操作（**找出僵死的子进程，释放子进程的PCB进程表项**），然后从wait中返回。**SIGCLD信号的作用是唤醒一个睡眠在<big>可被中断优先级上</big>的进程**。
     ...
     + 如果进程**忽略**该信号*(父进程中执行了`signal(SIGCHLD,SIG_IGN);`)*，那么系统调用wait的动作就有所不同，因为SIGCLD的作用仅仅是**唤醒一个**睡眠在可被中断优先级上的进程，那么执行wait调用的父进程被唤醒继续执行wait调用的后续操作，然后**等待其他的子进程**。

     + 如果一个进程**调用signal系统调用，并设置了SIGCLD的处理方法**，并且该进程有子进程处于僵死状态，则内核将向该进程发一个SIGCLD信号。
...
### 11.5.6 Linux的exit()和SIGCHLD信号(优质文章转载)
